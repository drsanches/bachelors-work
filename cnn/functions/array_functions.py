...
def input_image_scaling(image, input_size):
    ...
    final_image = Image.new('L', (input_size[0], input_size[1]), "White")
...
def input_array_scaling(array, input_size):
    image = array_to_image(array)
    image = input_image_scaling(image, input_size)
...
def deformation(image, size):
    new_image = scaling(image, size)
    new_image = cut_symbol(new_image)
    new_image = input_image_scaling(new_image, (image.width, image.height))
    return new_image